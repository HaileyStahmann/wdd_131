...
function search() {
    ...
    // Check recipe properties to see if the search keyword exists in any of them
    let matches = recipes.filter(recipe =>
        recipe.name.toLowerCase().includes(k) ||
        recipe.description.toLowerCase().includes(k) ||
        recipe.tags.some(tag => tag.toLowerCase().includes(k))
    );

    // Inject each recipe in match list into the html
...
function getRecipeTemplate(recipe) {
    ...
    const tags = tagTemplate(recipe.tags);
